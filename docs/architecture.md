...
Issue-Pilot 的核心架构（Reconciler → Dispatcher → Runner）本质上是一个通用的 **"感知 GitHub 状态变化 → 调度 AI Agent 执行"** 框架。当前 v1.0 仅实现了最小闭环：单 workflow、标签触发、Issue 分诊。
...
#### v1.0（当前）— 单 Workflow 分诊

**能力：** 一个 WORKFLOW.md，一种触发标签，Issue 分诊并留下评论。

**限制：** 只能做一件事。想让 AI 既做分诊又做实现，只能靠换 Workflow 文件重启进程。
...
v1.0 ──── v1.1 ──── v1.2 ──── v1.3 ──── v2.0 ──── v2.1 ──── v2.2
 ...
 Issue分诊   标签路由    自动串联  代码审查    看板驱动   /pilot    worktree
                                 PR合并
```
...
| **Workflow 文件是唯一的业务逻辑载体** | 框架代码不包含任何特定任务的逻辑（不硬编码"分诊"、"审查"等概念） |
