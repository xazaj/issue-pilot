...
Issue-Pilot 是一个本地运行的 AI 任务调度服务。它通过调和循环（Reconciliation Loop）定时查询 GitHub Issue 标签变化，发现符合条件的任务后，启动 Claude Code CLI 执行。全程串行，一次只处理一个 Issue，通过内存队列缓存多余任务。
...
- `pilot:ready` — 标记任务已就绪，等待 Agent 执行
- `pilot:in-progress` — 标记任务正在执行中
- `pilot:failed` — 标记任务执行失败，需人工介入
...
- `ready_label`：触发执行的 Issue 标签，默认 `pilot:ready`
- `in_progress_label`：认领时打上的标签，防止重复处理，默认 `pilot:in-progress`
- `failed_label`：失败时打上的标签，默认 `pilot:failed`
...
第 1 步 - 认领 Issue
  ...
  Octokit: 添加评论 "Task claimed by Issue-Pilot, starting execution..."
